"""Lobby arrival check-in tools.

These are the MCP-visible tools that Kitty uses to walk a new guest
through the front-desk security procedure in the Lobby.
...
def record_new_conversation(username: str) -> None:
    ...
    conn.execute("""
        INSERT INTO guests (username, visit_count, last_visit)
        VALUES (?, 1, ?)
...
async def register_guest(username: str, first_name: str):
    ...
    now = datetime.now().isoformat()
    conn.execute("""
        INSERT INTO guests (username, first_name, visit_count, last_visit)
        VALUES (?, ?, 1, ?)
        ON CONFLICT(username) DO UPDATE SET
            first_name = ?,
            visit_count = visit_count + 1,
