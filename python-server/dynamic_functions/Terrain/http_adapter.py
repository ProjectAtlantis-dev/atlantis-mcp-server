...
def coverage_cure_inventory(
    ...
    """Describe exact cure progress without treating cached paint as terrain."""
    ...
    rows = connection.execute(
        ...
        "       t.heightmap IS NOT NULL, c.source, c.version, x.source "
        "FROM tiles t "
    ...
    for row in rows:
        ...
        cured = exact_depth and bool(has_dem) and coastline_cured
        inventory.append(
            {
                "tile": tile_id,
                "depth": int(depth),
    ...
    return {
        ...
        "definition": (
            f"depth-{cure_depth} DEM plus current authoritative coastline mask"
        ),
        "summary": {
