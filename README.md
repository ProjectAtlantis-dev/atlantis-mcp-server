...
It holds the game/chat tools, bot runtime, static content under `Game/`, and live player state under `Data/`. The Atlantis MCP server treats it like any other dynamic-functions app: it scans the folder, exposes decorated functions as tools, and reloads them when files change.
